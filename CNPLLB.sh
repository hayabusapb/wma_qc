...
SID=$(sed -n "$SLURM_ARRAY_TASK_ID"p SID96.txt)
...
## New squashfs structure - Jan2022 Hutton
RR=/lustre03/project/6008063/ahutton/tractoflow_out/tractoflow_0.squashfs
...
#LOCAL=~/scratch/sdone_master/
LOCAL=~/scratch/sdone_master_0t200
...
then
	...
	singularity exec --overlay ${RR}:ro --bind $SD:/OUT $IMG trk2vtk_sls.py -i ${RT}/PFT_Tracking/${SID}_ses-2__pft_tracking_prob_wm_seed_0.trk -o /OUT/${SID}.vtk
 ...
 singularity exec --overlay $RR:ro --bind $OP:/Mount_OP $IMG cp -n $RT/Extract_DTI_Shell/${SID}_ses-2__dwi_dti.nii.gz $RT/Extract_DTI_Shell/${SID}_ses-2__bval_dti $RT/Extract_DTI_Shell/${SID}_ses-2__bvec_dti $RT/PFT_Seeding_Mask/${SID}_ses-2__pft_seeding_mask.nii.gz /Mount_OP
	...
	singularity exec --overlay ${RR}:ro --bind $IN:/Mount_in,$OUT:/Mount_Out $IMG xvfb-run-safe.sh tract2vol_Simple.sh /Mount_in $REF /Mount_Out


	##BINARY MASKS
        $cleanS

	singularity exec --overlay ${RR}:ro --bind $BOUT:/mnt_bout,$OUT:/mnt_out $IMG wm_mask2bin_Simple.sh /mnt_out /home/sliceruser/Slicer-4.11.0-2020-05-11-linux-amd64/Slicer /mnt_bout $REF
	...
	singularity exec --overlay ${RR}:ro --bind $BOUT:/mnt_bout,$MOUT2:/mnt_mout $IMG xvfb-run-safe.sh /home/conda/bin/pyradio_Simple.sh /mnt_bout $REF /mnt_mout/${SID}_FA_diffusion_measures.csv
	...
	rm $MOUT2/${SID}_FA_diffusion_measures.csv
       ...
       singularity exec --overlay ${RR}:ro --bind $BOUT:/mnt_bout,$MOUT2:/mnt_mout $IMG xvfb-run-safe.sh /home/conda/bin/pyradio_Simple.sh /mnt_bout $REF2 /mnt_mout/${SID}_RD_diffusion_measures.csv
	...
	$cleanS
       ...
       singularity exec --overlay ${RR}:ro --bind $BOUT:/mnt_bout,$MOUT2:/mnt_mout $IMG xvfb-run-safe.sh /home/conda/bin/pyradio_Simple.sh /mnt_bout $REF3 /mnt_mout/${SID}_MD_diffusion_measures.csv
	...
	singularity exec --overlay ${RR}:ro --bind $BOUT:/mnt_bout,$MOUT2:/mnt_mout $IMG xvfb-run-safe.sh /home/conda/bin/pyradio_Simple.sh /mnt_bout $REF4 /mnt_mout/${SID}_AD_diffusion_measures.csv
	...
	rm $MOUT2/${SID}_AD_diffusion_measures.csv
      ...
      singularity exec --overlay ${RR}:ro --bind $BOUT:/mnt_bout,$MOUT2:/mnt_mout $IMG xvfb-run-safe.sh /home/conda/bin/pyradio_Simple.sh /mnt_bout $REF5 /mnt_mout/${SID}_GA_diffusion_measures.csv
...
echo "DMPLL22.sh took: $((DURATION/60))min and $((DURATION%60))sec to execute"
